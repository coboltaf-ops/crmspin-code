      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          {permisos.editar && tab === 'registros' && (
            <button onClick={() => { setSelected(emptyProspecto(nextConsecutivo('PRS-', prospectos.map(p => p.codigo)).codigo)); setIsForm(true) }} style={{ ...btnStyle, background: '#0A5A5A', color: '#ffffff' }}>+ Nuevo Prospecto</button>
          )}
        </div>
      </div>
