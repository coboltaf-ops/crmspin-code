...
export type EmpresaInfo = { nombre?: string; nro_documento?: string; direccion?: string; ciudad?: string; logo_url?: string }
type ReportOptions = {
...
export function exportToPDF(opts: ReportOptions) {
  ...
  const logo = opts.empresa?.logo_url || ''
  const logoFmt = logo ? detectImageFormat(logo) : null
  ...
  if (logoFmt) {
    try { doc.addImage(logo, logoFmt, 8, 4, 28, 28) } catch { /* ignore bad logo */ }
  }
  doc.setTextColor(255, 255, 255)
  const xText = logoFmt ? 42 : 14
  if (empNombre) {
    doc.setFontSize(14); doc.text(empNombre, xText, 12)
    if (empSub) { doc.setFontSize(9); doc.text(empSub, xText, 18) }
    doc.setFontSize(13); doc.text(opts.title, xText, 28)
    if (opts.subtitle) { doc.setFontSize(9); doc.text(opts.subtitle, xText, 33) }
  } else {
    doc.setFontSize(16); doc.text(opts.title, xText, 14)
    if (opts.subtitle) { doc.setFontSize(10); doc.text(opts.subtitle, xText, 22) }
  }
  ...
  autoTable(doc, {
    ...
    margin: { left: 8, right: 8, top: headerH + 6 },
    tableWidth: 'auto',
  })
...
export function printReport(opts: ReportOptions) {
  ...
  const empBlock = empNombre ? `
    <div style="border-bottom:3px solid #1e3a8a;padding-bottom:12px;margin-bottom:16px;display:flex;align-items:center;gap:14px">
      ${logo ? `<img src="${logo}" alt="logo" style="height:60px;max-width:120px;object-fit:contain"/>` : ''}
      <div>
        <h1 style="color:#1e3a8a;font-size:20px;font-weight:800;margin:0">${empNombre}</h1>
  ...
  const html = `<!DOCTYPE html><html><head><title>${opts.title}</title></head><body style="font-family:Arial;padding:20px">
    ${empBlock}
    <h2 style="color:#1e3a8a">${opts.title}</h2>${opts.subtitle ? `<p style="color:#6b7280">${opts.subtitle}</p>` : ''}
    <table style="width:100%;border-collapse:collapse;margin-top:16px">
      <thead><tr>${opts.columns.map(c => `<th style="padding:8px 10px;background:#1e3a8a;color:#fff;font-size:11px;text-align:left">${c.header}</th>`).join('')}</tr></thead>
      <tbody>${rows}</tbody>
    </table>
